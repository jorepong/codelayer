...
> AI가 빠르게 만들고, 나는 그 코드에서 필요한 개념만 골라 내 것으로 만든다 — 탑다운 학습 프레임워크.
...
> **고정 저장소 없이, 아무 맥락에서나 배우고 싶다면 → [`learn` 스킬](https://github.com/jorepong/claude-code-skills/tree/main/learn).**
> CodeLayer는 학습 전용 저장소를 두고 그 안에서 프로젝트를 쌓아 가며 배우는 방식입니다. 반면 팀 프로젝트를 하다가, 또는 AI와 CI/CD를 논의하다가 방금 대화에 등장한 개념을 *그 자리에서* 배우고 싶을 때는 별도 저장소가 거추장스럽죠. 그럴 때는 같은 학습 엔진(개념 학습 · 구현 장악 두 트랙)을 떼어내 어디서든 쓸 수 있게 일반화한 `learn` 스킬을 씁니다 — `/learn [주제]` 한 줄로 호출하고, 학습 자료는 전역 위치 `~/.claude/learn/<주제>/`에 쌓입니다.

---

## 왜 탑다운 학습인가

전통적인 학습은 대개 **바텀업(bottom-up)** 이었습니다. 정해진 커리큘럼을 따라 기초 개념부터 차근차근 쌓아 올리고, 그 토대가 다져진 뒤에야 실제로 무언가를 만들었습니다. 이 방식은 지금도 유효합니다. 다만 한 가지를 전제합니다 — 무엇을 어떤 순서로 배울지가 이미 정해져 있고, 학습자는 그 순서를 따라간다는 것입니다.

AI 시대는 그 전제를 흔듭니다. 누구나, 언제든 부를 수 있는 뛰어난 1대1 튜터를 갖게 됐기 때문입니다. 이제는 정해진 커리큘럼을 앞에서부터 밟지 않아도, **지금 내게 필요한 것만 골라 그 자리에서 깊게 배울 수 있습니다.** CodeLayer는 이 가능성을 하나의 학습 방식으로 바꾼 도구입니다 — 바텀업을 **탑다운(top-down)** 으로 뒤집습니다.

탑다운에서는 순서가 반대입니다. 먼저 **내가 만들고 싶은 프로젝트를 구현합니다.** 그리고 그 프로젝트가 실제로 사용한 개념들 가운데 내게 없거나 약한 것만 **골라서** 배웁니다. 배울 것을 커리큘럼이 정해 주는 게 아니라, 지금 내가 만드는 것이 정해 줍니다. 필요가 학습을 이끌기에, 배운 것이 곧바로 내 프로젝트의 그 코드에 붙어 남습니다.

이 방식은 코딩에서 특히 값이 큽니다. AI는 프로젝트를 빠르게 구현해 주지만, 그 속도에는 함정이 있습니다 — 이른바 **바이브 코딩**에서는 '빠르게 돌아가는 결과물'이 강조될 뿐, 그 코드를 **내 것으로 만드는 일**은 쉽게 뒷전이 됩니다. 돌아가긴 하는데 왜 그렇게 돌아가는지는 모르는 코드가 쌓이는 것이죠. CodeLayer는 이 둘을 한자리에서 얻게 합니다. AI가 실무 수준으로 빠르게 만들고, 학습자는 그 코드에서 배우고 싶은 개념을 골라 내재화합니다. **빠른 개발과 '코드를 내 것으로 만드는 일'을 맞바꾸지 않아도 됩니다.**

> 이 구조는 한 번에 나오지 않았습니다. AI가 얕은 요약으로 넘어가지 않고 **충분한 깊이로 설명하도록 유도하는 규칙**을, 실제로 배워 나가며 계속 다듬어 만든 결과물입니다. 그 규칙 전문이 [`INSTRUCTIONS.md`](.codelayer/INSTRUCTIONS.md)입니다.
